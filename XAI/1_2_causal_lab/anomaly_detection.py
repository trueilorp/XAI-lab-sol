...
ALPHA = 0.05
TRAINING_FRAC = 0.7
PREFIX = "/home/trueilorp/xai-course/XAI/1_2_causal_lab/pepper_csv/"
TASK = "pepper"
...
def fit_normal_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray):
	...
	# Causal matrix axes: [parent_var, child_var, delay_tau]
	
	# Keep enough history so each parent lag is aligned with the child at time t.
	# normal_data.shape[0] --> total number of rows
	# causal_matrix.shape[2] --> number of lag slots (tau_max)
	max_time = normal_data.shape[0] - causal_matrix.shape[2]
	
	for var in np.unique(indices[1, :]):  # Iterate over child/target variables.
		# Get all parent variables and their delays for this target variable
		...
		# Save one coefficient vector per child variable, ordered like `var_indices`.
		fine_coeffs[var] = coeffs
		
		
		# example:
		# causal_matrix[2, 5, 1] != 0
		# causal_matrix[7, 5, 3] != 0
		# Then child 5 depends on:
		# parent 2 from 1 step ago: X_2(t-1)
		# parent 7 from 3 steps ago: X_7(t-3)
		# X_5(t) ≈ a1*X_2(t-1) + a2*X_7(t-3) + bias
