...
class SR_DQN(OffPolicyAlgorithm):
	...
	def _get_random_action(self, observation):
		...
		# print(f"Suggested actions: {actions}") # 0,1,2,3
		# print("Action space: ", self.action_space) # Discrete(7)
		
		# normalize weights and sample action 
		# if no suggested action, sample from uniform distribution
		if actions:
			weights_actions = [0] * self.action_space.n
			for i in range(self.action_space.n):
				if i in actions:
					weights_actions[i] = self.conf_level # assign weight equal to conf_level to suggested actions
				else:
					weights_actions[i] = 1 - self.conf_level # assign weight equal 1-conf_level to others
			weights_actions = np.array(weights_actions) / np.sum(weights_actions) # normalize weights
			return random.choices(range(self.action_space.n), weights=weights_actions, k=1)[0]
		else:
