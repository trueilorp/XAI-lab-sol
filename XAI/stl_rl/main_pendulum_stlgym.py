...
for use_this_seed in seeds_to_inspect:
	...
	for i_episode in range(1, MAX_EPISODES + 1):
		...
		while num_steps < EPISODE_LENGTH * EPISODE_PER_BATCH:
			...
			for t in range(EPISODE_LENGTH):
				...
				# Reward shaping * 5
				# reward += cost_thetadot/20 + cost_torque/20
				
				mask = 0 if t == EPISODE_LENGTH - 1 else 1
				
				# Reward shaping * 5
				memory.push(state, np.array([action]), mask, next_state, reward + cost_thetadot/100 + cost_torque/100, cost_thetadot, cost_torque)
	...
	# Save rewards
	np.savez(f"results/pendulum/stlgym_{START_SAFETY}_{use_this_seed}.npz", 
